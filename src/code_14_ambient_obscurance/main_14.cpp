...
int g_buffer_size_x, g_buffer_size_y, n_blur_steps;
float radius;
...
static int fps;
static glm::vec3 ambient_term = glm::vec3(0.5f, 0.5f, 0.5f);
static glm::vec3 diffuse_term = glm::vec3(0.5f, 0.5f, 0.5f);
...
void gui_setup() {
	...
	if (ImGui::BeginMenu("parameters"))
	{
		...
		ImGui::ColorEdit3("ambient term", (float*)&ambient_term, misc_flags);
		ImGui::ColorEdit3("diffuse term", (float*)&diffuse_term, misc_flags);
		...
		ImGui::SliderInt("n blur steps", &n_blur_steps, 0, 4);
		...
		ImGui::SliderFloat("radius", &radius,0.0,.001f,"%.6f");
...
static void writeOFF(const std::string& filename, const std::vector<glm::vec3>& points)
{
...
void ssaoKernel() {
	...
	for (int i = 0; i < 256; ++i)
	{
		float theta = 2.0f * pi * randomFloats(gen);
		float u = randomFloats(gen);
		float cosAlpha = std::cos(alpha);
		float cosPhi = 1.0f - u * (1.0f - cosAlpha);
	...
	writeOFF("kernel.off", ssaoKernel);
...
int main(int argc,char**argv)
{
	...
	blur_shader.create_program((shaders_path + "fsq.vert").c_str(), (shaders_path + "blur-avg.frag").c_str());
	...
	renderable r_cone = shape_maker::quad();
	renderable r_box = shape_maker::cube();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindTexture(GL_TEXTURE_2D, fbo.id_tex);
	 	// glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, g_buffer_size_x, g_buffer_size_y, 0, GL_RGBA, GL_FLOAT, NULL);
...
//glViewport(0, 0, 1000, 800);
		...
		glUniformMatrix4fv(g_buffer_shader["uT"], 1, GL_FALSE, &stack.m()[0][0]);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, r_cone().ind);
		glDrawElements(r_cone().mode, r_cone().count, r_cone().itype, 0);
		stack.pop();

		stack.push();
		stack.mult(glm::scale(glm::mat4(1.f), glm::vec3(0.1, 0.1, 0.1)));
		glUniformMatrix4fv(g_buffer_shader["uT"], 1, GL_FALSE, &stack.m()[0][0]);
		r_box.bind();
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, r_box().ind);
		glDrawElements(r_box().mode, r_box().count, r_box().itype, 0);
		stack.pop();
		*/
...
//		glDrawBuffers(1, bufferlist);
		...
		float scale = r_objs[0].bbox.diagonal() *pow( glm::determinant(stack.m()), 1.f/3.f);

		glUniform1i(ao_shader["uDepthMap"], 0);
		glUniform1i(ao_shader["uNormalMap"], 1);

		glUniform1f(ao_shader["uRadius"], radius);
		glUniform1f(ao_shader["uModelScale"], scale);
...
//  glViewport(0, 0, g_buffer_size_x, g_buffer_size_y);
 ...
 //  goto swapbuffers;
		...
		glUniform3f(final_shader["uAmbient"], ambient_term.x, ambient_term.y, ambient_term.z);
		glUniform3f(final_shader["uDiffuse"], diffuse_term.x, diffuse_term.y, diffuse_term.z);
		glActiveTexture(GL_TEXTURE0);
 		glBindTexture(GL_TEXTURE_2D, fbo.id_tex1);
		glActiveTexture(GL_TEXTURE1);
