...
char diffuse_map_name[65536] = { "./textures/brick_wall2-diff-512.png" };
char displacement_map_name[65536] = { "./textures/brick_wall2-diff-512.png" };
char normal_map_name[65536] = { "./textures/brick_wall2-nor-512.png" };
char model_name[65536] = { "./models/troll.glb" };
...
void gui_setup() {
	...
	if (ImGui::BeginMenu("Model")) {
		...
		if (ImGui::Selectable("loaded model", selected_mesh == 1)) selected_mesh = 1;
		if (ImGui::Selectable("torus", selected_mesh == 2)) selected_mesh = 2;
		ImGui::EndMenu();
	}
...
int main(void)
{
	...
	window = glfwCreateWindow(width, height, "code_12_texture_mapping", NULL, NULL);
	...
	/* create a torus*/
	shape s_torus;
	shape_maker::torus(s_torus,0.5,1,30,30);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		case 2:	r_torus.bind();
			glDrawElements(r_torus().mode, r_torus().count, r_torus().itype, 0); break;
		case 1:
			stack.push();
