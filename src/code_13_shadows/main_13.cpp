...
renderable r_frame, r_plane,r_line,r_torus,r_cube, r_sphere,r_quad,r_cylinder;
...
void draw_cylinder(shader & sh) {
	glUniformMatrix4fv(sh["uT"], 1, GL_FALSE, &stack.m()[0][0]);
	r_cylinder.bind();
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, r_cylinder().ind);
	glDrawElements(r_cylinder().mode, r_cylinder().count, r_cylinder().itype, 0);
}

void draw_scene(  shader & sh) {
	if (sh.has_uniform("uDiffuseColor")) glUniform3f(sh["uDiffuseColor"], 0.4f, 0.5f, 0.4f);
	stack.push();
...
int main(void)
{
	...
	window = glfwCreateWindow(width, height, "code_16_shadows", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
	...
	/* create a cylinder */
	shape s_cylinder;
	shape_maker::cylinder(s_cylinder,50);
	s_cylinder.to_renderable(r_cylinder);
